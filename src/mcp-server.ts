...
server.tool(
  ...
  'Batch open Dependabot PRs into one integration branch, validate each merge, and open a single PR to the base branch.',
  {
    ...
    maxPrs: z.number().int().positive().default(20).describe('Maximum Dependabot PRs to process'),
    cursorApiKey: z.string().optional().describe('Cursor Cloud API key for failure explanations'),
  },
  async (params) => {
    const { token, cursorApiKey, ...configFields } = params;
